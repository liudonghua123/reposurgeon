...
Repository head::
     print command now allows input redirection (input from file or heredoc)
     Minor improvements in baton handling.
     repocutter pathrename REs are constrained to match full oath segments
     --nobranch is retired; use repocutter to prepare flat repositories

4.30: 2021-10-13::
     Speed up DoTagify by parallelizing it.
     =J selection gets all commmits and tags with non-ASCII metadata
...
4.25: 2021-03-02::
     ...
     Revert removal of --cvsignores. Nay be revisyted in a future release
...
4.24: 2021-02-13::
     ...
     Subcommand moves to before first argument in branch/path/tag/reset commands.
...
4.22: 2020-12-28::
     ...
     Many improvements to the documentation, including improved cross-references.
