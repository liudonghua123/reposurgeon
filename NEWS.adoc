...
Repository head::
     ...
     "help" without topic now ermits a custom TOC with command groupings.
     Unite now produces union repositotries with a valid master branch.

4.24: 2021-02-13::
     Retire the read --cvsignores option; best to use an expunge instead.
     Subversion proerties discarded during read are logged, not shouted.
     ...
     The "paths [sub|sup]" command have been replaced by "path rename".
