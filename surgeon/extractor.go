...
// assume logit(), croak(), throw() and catch() do sane things.
...
func (cm *ColorMixer) simulateGitColoring(mc MixerCapable, base *RepoStreamer) {
	...
	for _, refname := range base.refs.keys {
		if logEnable(logTOPOLOGY) {
			logit("outside branch coloring %s %s", base.refs.get(refname), refname)
		}
...
func (cm *ColorMixer) _branchColor(rev, color string) {
	...
	if logEnable(logTOPOLOGY) {
		logit("inside branch coloring %s %s", rev, color)
	}
	...
	if logEnable(logTOPOLOGY) {
		logit("%s is-a-branch is %v", color, isBranchColor)
	}
	unassigned := func(rev string) bool {
		u := (cm.base.meta[rev].branch == "")
		if logEnable(logTOPOLOGY) {
			logit("%s unassigned is %v", rev, u)
		}
		return u
	}
	...
	for {
		...
		var parents []string
		if logEnable(logTOPOLOGY) {
			logit("parents of %s (%s) before filtering %v", rev, timestamp.UTC(), cm.base.getParents(rev))
		}
		...
		if logEnable(logTOPOLOGY) {
			logit("parents of %s are %v", rev, parents)
		}
...
func (he *HgExtractor) gatherChildTimestamps(rs *RepoStreamer) map[string]time.Time {
	...
	for _, h := range items.keys {
		...
		if logEnable(logTOPOLOGY) {
			logit("setting default branch of %s to %s", h, branch)
		}
		rs.meta[h].branch = branch
...
func (he *HgExtractor) colorBranches(rs *RepoStreamer) error {
	...
	if colorItems != nil {
		...
		for _, h := range colorItems.keys {
			...
			if logEnable(logTOPOLOGY) {
				logit("setting branch from color items, %s to %s", h, color)
			}
...
func (rs *RepoStreamer) extract(repo *Repository, vcs *VCS) (_repo *Repository, err error) {
	...
	isBranchColor := func(name string) bool {
		for rev, commit := range rs.commitMap {
			if name == commit.Branch && rs.refs.get(name) == rev {
				return true
			}
		}
		return false
