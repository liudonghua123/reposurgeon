...
func (rs *Reposurgeon) newLineParse(line string, parseflags uint, capabilities orderedStringSet) *LineParse {
	...
	if !isDefined(rs.selection) && (parseflags&parseALLREPO) != 0 {
		rs.selection = rs.chosen().all()
	}
	if isDefined(rs.selection) && (parseflags&parseNOSELECT) != 0 {
		panic(throw("command", "command does not take a selection set"))
	}
	if !isDefined(rs.selection) && (parseflags&parseNEEDSELECT) != 0 {
...
func (rs *Reposurgeon) DoGraft(line string) bool {
	...
	var graftPoint int
	if isDefined(rs.selection) && rs.selection.Size() == 1 {
...
func (rs *Reposurgeon) DoTag(line string) bool {
	...
	if verb == "create" {
		...
		var ok bool
		var target *Commit
		if isDefined(repo.named(tagname)) {
	...
	if verb == "move" {
		...
		if isDefined(repo.named(newname)) {
