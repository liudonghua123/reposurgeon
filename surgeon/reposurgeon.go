...
stream files. Here's how reposurgeon gathers hints from stream files:
...
2. Streams generated by src(1) are identified as src, SCCS, or RCS.
This is considered a strong hint.

3. File basenames that match those used by known version-control
...
4. Certain magic $-headers in content blobs are considered 
weak hints. These are associated with SCCS, RCS, and CVS.

The sourcetype in a stream not from a live repository is set by the
...
sequence consisting only of deletes crafted to prevent the importd
...
Clears Q bits, than sets thw Q bits of every importd commit.
`)
}

// CompleteImport is a completion hook over import modes
func (rs *Reposurgeon) CompleteImport(text string) []string {
	return []string{"--after", "--date", "--firewall="}
}

// DoImport creates a new commit from a tarball.
func (rs *Reposurgeon) DoImport(line string) bool {
	parse := rs.newLineParse(line, "import", parseREPO, orderedStringSet{"stdin"})
	defer parse.Closem()
	repo := rs.chosen()
	if !rs.selection.isDefined() {
	...
	repo.doImport(tarballs, commit, strip, parse.options.Contains("--firewall"), parse.options.Contains("--after"), date)
	return false
}
