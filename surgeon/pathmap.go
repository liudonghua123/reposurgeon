...
// PathMapDirs is a specialized kind of PathMap where you can also create an
// empty directory, and where empty directories are not automatically removed.
// Warning: for PathMapDirs, isEmpty() returns true iff there are neither files
// nor empty directories.
type PathMapDirs struct {
    PathMap
}

func newPathMapDirs() *PathMapDirs {
	return &PathMapDirs{*newPathMap()}
}

// A version of snapshot() that returns a PathMapDirs
func (pmd *PathMapDirs) snapshot() *PathMapDirs {
	return &PathMapDirs{*pmd.PathMap.snapshot()}
}

// A version of copyFrom() that accepts PathMapDirs
func (pmd *PathMapDirs) copyFrom(targetPath string, sourcePathMap *PathMapDirs, sourcePath string, srcid string) {
	pmd.PathMap.copyFrom(targetPath, &sourcePathMap.PathMap, sourcePath, srcid)
}

// A version of PathMap.remove() that does not get rid of empty directories
func (pmd *PathMapDirs) remove(path string) {
	parts := strings.Split(path, svnSep)
	dir, name := parts[:len(parts)-1], parts[len(parts)-1]
	parent := &pmd.PathMap
	for _, component := range dir {
		if subtree, ok := parent.dirs[component]; ok {
			// The component exists. Unshare it so that the deletion
			// does not affect other PathMaps.
			subtree = subtree._unshare()
			parent.dirs[component] = subtree
			parent = subtree
		} else {
			// The path does not exist, bail out
			if logEnable(logWARN) {
				logit("component %q to be deleted is missing", component)
			}
			return
	...
	// Now we remove the last component from its containing PathMap
	delete(parent.dirs, name)
	delete(parent.blobs, name)
}

// mkdir creates an empty PathMap at the given path, creating missing hierarchy
func (pmd *PathMapDirs) mkdir(path string) {
	pmd._createTree(strings.Split(path, svnSep))
}
