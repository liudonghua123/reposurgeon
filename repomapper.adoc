...
When moving a repository from a centralized to a distributed system,
therefore, one of the prerequisites is comopiling a contributor map
that associates each account name on the old system to a DVCS-style ID
...
when a password file is processed.
...
is used to fill in the entry. If there are multiple matches the last is
kept.
