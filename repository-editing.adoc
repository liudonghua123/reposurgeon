...
CVS has top-tier support with good testing. Iy has been used for many
CVS-to-Git conversions.

It is not expected that export to CVS will ever be implemented. (This
...
of any of the comversion paths.
...
undiscovered data-model issues here.
...
Pijul, which is still new and in intensive development as of 2023, will be
supported when it grows an importer/exporter pair for Git fast-import
streams.
...
Run "<<repotool.adoc#,repotool makemake>>" in the scratch directory;
it requires that you follow the makemake verb with a project name.
...
is robust and well-tested. Normally reposurgeon will do branch
analysis for you.  On most Subversion repositories, and in particular
anything with a standard trunk/tags/branches layout, it will do the
right thing. It will also cope with adventitious branches in the root
directory of the repo, such as some projects use for website content.
...
. SCCS, RCS, CVS, SVN: Map author IDs from local to DVCS form.

. SVN: Check for leftover cvs2svn junk commits and remove them if possible.

. Lift references in commit comments.

. Massage comments into summary-line-plus-continuation form.

. If the project used the GNU ChangeLog convention, run "<<changelogs_cmd>>".

. CVS, SVN: Remove empty and delete-only tip commits where appropriate.

. CVS, SVN: Review generated tags, pruning and fixing locations as appropriate.

. Look for branch merge points and patch parent marks to make them.

. CVS: Resolve any <<zombie>> files in a CVS conversion by patching in D ops.

. If there's a root branch, check for and remove junk commits on it.

. Use the transcode command to fix up metadata in non-UTF8 encoding.

. Run lint to detect remaining anomalies that might need to be patched.

. CVS, SVN: Fix up or remove $-keyword cookies in the latest revision.

. For the record, make a tag noting time and date of the repo lift.

. If your target was git, run  `git gc --aggressive`.
...
More about common forms of cruft:
...
Tags from CVS -root commits::
    ...
    often persist through up-conversions to SVN.  "tag delete /-root$/"
    will clean those up; this is not done automatically
...
Branch tip deletes, deletealls, and unexpressed merges::
    ...
    svn:mergeinfo properties as clues.  Reposurgeon tries to interpret
    svn:mergeinfo properties to create these merges automatically, but
    Subversion merging is tricky enough that these proprties are often
    not set when they should have been.
...
You can find empty commits as the =Z visibility set and clean them up
with the command `<<tagify_cmd>>`, or simply delete them.

If your repository is small, a good way to spot junk commits is to
eyeball the picture of the commit DAG created by the reposurgeon
...
See the documentation of the `<<stampify_cmd>>` command for details
on how fix up Subversion and CVS changeset references in comments so
they're still meaningful.

The command `=L <<msgout_cmd>>` is good for extracting commits with
multi-line comments in a form that can be deleted and merged back in
with the msgin command.

The reposurgeon command `=H <<list>> inspect` will show you tip commits
which may contain only deletes and deletealls.

Tags can be inspected with `=T <<list>> inspect`. Junk tags can be
removed with the `<<delete_cmd>>` command. Tag comments can be
modified with a msgin/msgout sequence.  Check that the creation date of tags
...
The command `=I` will select all commits with attributions and
comments that don't decode to UTF-8 in both the commit comment and
attribution parts.  You can eyeball those to figure out what the
encoding is and apply the `<<transcode_cmd>>` command to fix things.

Reposurgeon has a `<<merge_cmd>>` command specifically for performing
branch merges.  The `<<msgin_cmd>>` command will also allow you to add
...
keyword cookies lest they become unhelpful relics. The full Subversion
...
A comman pitfall is to expect CVS tags to become tag objects in
reposurgeon and expect to be able to rename or delete them with the
"tag" command. CVS does not have annotated tags, they get mapped to
lightweight tags by cvs-fast-export, which are actually resets;
you have to manipulate them with the "reset" command instead.
...
"<<repotool.adoc#,repotool>> makemake", mention that in your bug
