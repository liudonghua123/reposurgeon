...
named '```unbranched```'. If there is no "trunk" (or rather no master
branch), then this synthetic '```root```' branch becomes the master
branch. You can map another directory to master using
`<<branchify_cmd>>` and `<<branchmap_cmd>>`. If the Subversion
repository has a branch named '```unbranched```' the name
