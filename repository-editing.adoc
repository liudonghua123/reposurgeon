...
lightweight tags by cvs-fast-export, which are actually branchlets;
you have to manipulate them with the "branch" command instead.
...
In theory reposurgeon should fakk back to referencing the latest
...
repocutter emits warnings on stderr when it writes a stream with
missing copyfrom sources, but reposurgeon will crash when presented
with such a stream. If you have a conversion pipeline that uses
repocutter, the right first step is to review that pipeline carefully.
Make sure you haven't discarded standard error, and look for
"missing copyfrom source" mewssages indicating a missing copyfrom revision.
