...
Note that this will *not* flag and detect commmits with metadata in
ISO 8859 single-byte encodings, which can't be distinguished from
Unicode Latin-1 by looking at the bytes.  You can find these with the
`=J` selection
...
Another useful tool is the repocutter "swapsvn" primitive.
