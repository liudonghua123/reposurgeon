...
. The distinction between things done with policy flags vs. things
  done with subcommand ketwords is not very principled. Probably
  we should use subcommand keywords more for alternative modes, as
  opposed to policy bits that can be set several at a time.
