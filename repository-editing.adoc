...
//
// Each inclusion is merged with either TOPIC or COMMAND.  TOPIC
// is used by the script that generates the browsable form of
// the documentation. COMMAND is not currently used, it's there so
// all inclusions are marked and it's easier to check that there
// are no missing markers.
...
Then read the <<quickstart>> section (immediately following the
...
continue by reading <<conversion>>.  It is not necessary to read the
...
about a particular command or group of commands.
...
If you find an error or omission in this document, you can suggest an
edit https://gitlab.com/esr/reposurgeon/-/blob/master/repository-editing.adoc[here].
...
and untested read support for Monotone. See <<support> for more details.
...
Perforce (p4) is not directly supported, but the auxiliary program
repotool(1) can mirror a p4 repository as a local git repository.
...
This will read repository into memory from `projectdir` and check it
...
You can use "help" and "help <command>" to learn more about
`reposurgeon` commands, or just read on.

Typically you put these commands in a script that you evolve by
experimenting until you got a conversion that suits your tastes and
needs. `"script"` command allows to read such command list from a file.

--------------------------
$ reposurgeon "do projectscript.lift"
--------------------------

As a motivating example, the script below does a Subversion-to-Git
...
# Ensure that all author dates are unique, bunping where necessary. Then
...
You can find more usage examples in <<tipsntricks>>.

[[support]]
== Supported Version Control Systems

The design objective of reposurgeon is to support history
...
The internal representation reposurgeomn uses for repository states
...
Reposurgeon can import, but not export, CVS repositories.  You must have
...
Beware that the quality of CVS lifts may be poor, with individual
...
See <<CVS>> for discussion and workarounds.

CVS has top-tier support with good testing. It has been used for many
CVS-to-Git conversions.

It is extremely unlikely that export to CVS will ever be implemented. (This
means: somebody would have to pay the maintainer real money to make it
happen.)

=== Subversion

Reposurgeon can import Subversion dumpfiles or repositories. If you want
...
also translate cleanly.

Operator errors, prior conversion with tools like cvs2svn, and
gatewaying with git-svn can leave cruft in a Subversion history
that needs to be cleaned out.  See <<subversion>> for discussion
and workarounds.

Subversion has top-tier support with excellent testing. It has been
used for many, many Subversion lifts to Git; this is the best-tested
of any of the conversion paths.

It is not likely that export to Subversion will ever be
...
Mercurial (hg): reposurgeon can import and export Mercurial
repositories; see <<mercurial>> for details. Reposurgeon has been used
for successful history conversions from Mercurial to Git.

Mercurial has top-tier support, but only basic test coverage.

See <<mercurial>> for more details.

=== Bazaar/Breezy

Bazaar and Breezy (bzr, brz): These are workalikes. Import and export
have only basic test coverage, but reposurgeon has been successfully
used for a major Bazaar-to-Git conversion (the history of Emacs).

Some Bazaar extensions - per-commit properties and multiple authors -
...
Bazaar has top-tier support, and Breezy is expected to replicate its
behavior closely enough to inherit that. Test coverage is only basic,
however.

=== Second-tier systems

A VCS is "second tier" if import capability is checked in the
reposurgeon test suite, but it hasn't had known use for a
conversion in the wild.

RCS: reposurgeon will read an RCS collection.  It uses
http://www.catb.org/~esr/src[SRC] to get and export a fast-import
stream to reposurgeon's importer. This importer does not attempt to
make changesets out of cliques of per-file commits; you'll have to dp
this yourself using reposurgeon's coalesce command.

SRC: reposurgeon can read an SRC collection. The caveats for RCS apply.

Writing to the file-oriented SRC and RCS systems can be done via
...
SCCS: reposurgeon will read an SCCS collection. It uses
http://www.catb.org/esr/src/[SRC] to get and export a a fast-import
stream to reposurgeon's importer. This importer does not attempt to
make changesets out of cliques of per-file commits; you'll have to dp
this yourself using reposurgeon's coalesce command.

Fossil: reposurgeon declares an importer-exporter pair, and will read
...
and author fields - that distinction will be lost if you export to it.

darcs: reposurgeon declares an importer-exporter pair, but the
...
in case it is necessary to rescue a legacy darcs history.

=== Third-tier systems

These have no coverage in the test suite.  Significant issues can be
expected.

BitKeeper (bk): As of version 7.3 (and probably earlier versions under
...
At one point reposurgeon was used to convert a Bitkeeper history (that
...
later built in. The quality of the built-in exporter is unknown.

Monotone (mtn): reposurgeon declares an importer-exporter pair, but the
...
it may be dropped in a future release.

=== Indirect support

Perforce (p4): repotool can be used to mirror a remote p4 repository as a
...
most developed appears to be called "ac2git"]; you should be able
...
https://pijul.org/[Pijul], which is still new and in intensive
development as of late 2023, will be supported when it grows an
importer/exporter pair for Git fast-import streams.

Arch, ArX, DCVS: these systems were open-source but have been dead
...
support for these and no plan to write any.

QVCS is open source but (as of late 2023) has been moribund since
2010. There is no support and no plan to write any.

Code Coop: open-source but undocumented. As of late 2023 it has been
untouched since 2018.  There is no support and no plan to write any.

AutoDesk Vault, Azure DevOps Server, CADES, Dimensions CM, ClearCase,
...
support, unless someobody with a lot of money decides to pay for it.

For other systems, see the archived page on
https://git.wiki.kernel.org/index.php/InterfacesFrontendsAndTools#Interaction_with_other_Revision_Control_Systems[Git
wiki page on conversion tools]. Warning: the information on it may be
out of date.
...
older or weirder, see the section on other VCSes for some hints,
...
Run "<<repotool.adoc#,repotool initmake>>" in the scratch directory;
...
can't make rsync work.

Note by the way that repotool mirror's "rsync:" URLs do *not* have the
...
host plus path rsync source specification.

If what you have is a Subversion checkout directory, the following
command will tell you the URL for its source repository. You may

--------
svn info --show-item repos-root-url
--------

If your first access attempt failed with some message about access
control, you may need to uncomment the RUSERNAME and RPASSWORD and
set them to a valid credential pair.
...
repo you're converting. Each line should be in the following form:
...
Install whatever front end reposurgeon needs to read your repository.
...
have a tool you need, repoosugeon will bail out gracefully and inform
you what's missing.

The generic-workflow Makefile will call reposurgeon for you,
...
repositories it could be more than a few minutes.

This will convert your repository to git. If you need to export
to something else, reposurgeon has write support for swveral
other modern VCSes.
...
On the other hand, while Subversion conversion has its complexities it
...
'```<<view_cmd>>```' command to examine the conversion, looking (in
...
lot of cruft and conversion artifacts in it.

Here's a checklist of cleanup steps, which we'll expand on later in
this section. If you're using the makefile generated by repotool, some
of these will be done by commands in your lift script.

==== Apply your author map

This step applies to SCCS, RCS, CVS, and SVN. Map author IDs from
local to DVCS form.  The reposurgeon command for this is "authors
read"; see '```<<authors_cmd>> read```'.

==== Mine changelogs

If you don't know what a GNU ChangeLog is you can skip this step.

If you're converting form a VCS that doesn't have separate committer
and author attributions (e.g. CVS, SVN, Fossil), and the project
you're converting used the GNU ChangeLog convention, include
"<<changelogs_cmd>>" somewhere in your lift script to capture
...
==== Clean up after legacy character sets

You may have metadata in your repository in an encoding incompatible
with UTF-8. The most common glitch of this kind is Latin-1 chracters
in committer names.

Unfortunately, this process can't be automated; the fact that the
...
to the development of Unicode and UTF-8.

The selector `=I` will choose all commits with attributions and
comments that don't decode to UTF-8 in both the commit comment and
attribution parts. You can use `=I list inspect` to view them.

It's up to you to figure out what the encoding is and apply the
'```<<transcode_cmd>>```' command to re-encode in UTF-8. You
can test the results of applying different encodings 

Example commands:

--------
# Try out latin1 decoding to see if it throws errors
=I msgout --decode=latin1

# Apply latin-1 decoding to UTF-9
=I transcode latin1
--------

==== Convert ignore files

If you're using reposurgeon for a CVS- or Subversion-to-git conversion
cvs-fast-export and reposugeon will convert ignore patterns for you
...
Any .gitignore files found in a Subversion repository *before
conversion* were almost certainly created by git-svn users ad hoc and
...
reversed with the `--user-ignores` option, which will retain that
information and merge it with the ignore patterns generated from
svn:ignore and svn:global-ignores properties.
...
Your conversion may contain mechanically-generated tags (both
lightweight and annotated) that convey no actual information.  There
are clutter and should be removed
...
CVS makes tags that are branch root markers for internal
...
SVN commits with no fileops are automatically transformed into
...
cvs2svn. This command will clean these up:
...
Importers may generate commits that are emoty (have no fileops) for
various reasons. You will probably want to delete these; they're
preserved just in case something about the metadata is interesting.
...
You can save any that have interesting comments as tags attached
to their parent commit with '```<<tagify_cmd>>```'.

The rest of this instruction step applies only if you are
convertung a Subversion repository.

Junk generated by *cvs2svn* to carry tag information lurk in the
...
you just caught them with "=Z list".

Less commonly, generated junk commits have long lists of spurious
delete fileops.  These can be listed with the =D selector:

--------
=D list
--------

The trickiest kind has actual file content duplicating parent file
...
comments that are empty or consist of the string "*** empty log
message ***",

Either <<view_cmd>> or <<graph_cmd>> can be q good way to spot junk commits.
...
places. Be aware that the <<graph_cmd>> command outputs DOT, the
language interpreted by the http://www.graphviz.org/[graphviz] suite;
...
==== Fix up references in commit comments

Different version-control systems have different conventions for how
a commit comment can refer to a previous commit. Modern DVCSes
useually use opaque hexadecimal strings that express some hash
...
What all these have in common is that they stop being usefuk when you
move your history to a new version-control system.  There are
two different ways to deal with this, and you need to make a
policy decision about which fits your needs.

One way is to use '```<<append_cmd>>```' to add a line to each commit
giving its identifier. This allows you to chace references by
...
archives) that you want to stay valid.

The other way is to convert these references into action stamps -
identifiers that are independent of the VCS you are in,  This has the
advantage of being future-proof and cluttering your conversion less. 

You due the latter thing with '```<<stampify_cmd>>```',  But there
is a preparation step you have to do first, which is to hack the
exising references into reference cookies that
'```<<stampify_cmd>>```' can see.

There's a detailed of this process and its commands at
<<reference-lifting>>.

==== Massage comments into summary-line-plus-continuation form.

git and hg both encourage comments to begin with a summary line that
can stand alone as a short description of the change; this practice
produces more readable output from `git log` and `hg log`.

For a really high-quality conversion, multi-line comments should be
edited into this form. Some of this can be done with,
'```<<gitify_cmd>>```', but you'll need to go through and fix
remaining cases by hand.  The =L selector is good for finding these.

Here is a workflow for this cleanup step:

1. Do "=L msgout --id >message.box" to dump comments that lack
a spacer line after the first to a mailbox file.  The actual name
of the mailbox file doesn't matter of course.

2. Use your preferred text editor to fix up the comments.

3. Do "msgin <message.box" to apply the fixes.

==== Patch in missing branch merges.

You can skip this step if you're converting from a DVCS, it will
already have branch merges expressed.  But CVS has no concept of
mering branches at all. Under Subversion, eposurgeon tries to
...
not set when they could have been.

You can review your branch tip commits with the =H selector. By
...
about this at <<subversion>>).

You can use '```<<merge_cmd>>```' to fix these up. This is safe
because adding a merge link doesn't modify any later content;
it's only used for ancestry tracking when computing diffs.

====  Subversion only: Review branch tip deletes and deletealls

In Subversion it is common practice to delete a branch directory when
...
In a DVCS, deletes at a branch tip don't save you any storage, so it
makes more sense to leave the branch with all of its tip content live
if you're not going to delete it entirely.  

It;s a judgment call whether to delete these. They could be considered
clutter, or they could be considered documentation of the fact that
development on a branch has closed.

==== CVS only: Resolve zombie files

CVS conversions occasionally have <<zombie>> files - that is, filesa
which persist in conversion commits after timestampted they were
deleted in the CVS repository.

The easiest way to detect these is to run a trial conversion with
...
There is a more detailed discussion of this issue at <<CVS>>.

==== Subversion only: check for a root branch

After conversion of a branchy Subversion repository, look to see if
...
belong to, they'll wind up there.
...
If this happens, it's likely those commits will be junk.  It's up to
you to delete or transplant them appropriately.

==== Fix up or remove $-keyword cookies in the latest revision.

One minor feature you lose in moving from SCCS, RCS, CVS, Subversion,
...
You should go through the last revision of the code and remove `$Id$`,
`$Date$`, `$Revision$`, and other keyword cookies lest they become
unhelpful relics. The full Subversion set is `$Date$`, `$Revision$`,
`$Author$`, `$HeadURL$` and `$Id$`. CVS uses `$Author$`, `$Date$`,
`$Header$`, `$Id$`, `$Log$`, `$Revision$`, also (rarely) `$Locker$`,
`$Name$`, `$RCSfile$`, `$Source$`, and `$State$`. A command like `grep
-R '$[A-Z]' .` may be helpful.

==== Run lint to detect remaining anomalies

Run <<lint_cmd>> to detect remaining anomalies that might need to be
patched.

==== Record the conversion into the history
...
==== Garbage-collect the conversion

If your target was git, run  `git gc --aggressive`. This can reduxe it
in size considerably.

[[conversion-comments]]
==== A note on conversion comments
...
[[recovering]]
==== A note on recovering from errors
...
over 80.  The Linux kernel developers like to stay at 65 or under.
...
given as arguments on the reposurgeon invocation line.
...
Commands that modify the history don't normally generate summaries
...
Note that this means the old repository can be left in service while
the conversion recipe is under development. Recipe development should
be treated as a serious project with its own change tracking.
...
To give you the flavor of the command language, here
...
// TOPIC
include::docinclude/syntax.adoc[]
...
Regular expressions are an important building block of
...
| E | earliast commit on each branch   |
| F | all fork (multi-child) commits   |
...
| Q | Recently touched                 | Set/cleared by many commands
| R | all resets                       |
| T | all tags                         |
...
|===================================================================
| type                           | interpretation
| RFC3339 timestamp              | commits or tags with that time/date
...
only the second in the set (by position in the stream file).
...
paths::
   ...
   '```R```', '```C```', '```N```'.
+
If the first character in a path expression is '```~```', the path
...
// TOPIC
include::docinclude/selection.adoc[]

// TOPIC
include::docinclude/functions.adoc[]

// TOPIC
include::docinclude/operators.adoc[]

[[redirection]]
=== Redirection and shell-like features

// TOPIC
include::docinclude/redirection.adoc[]

[[qbits]]
=== Q bits

Each event has an associated "Q" bit that says whether it was touched
...
generators, such as the "lint" command.

Thus, "=Q count" and "=Q list" can provide you with a check on the
...
reposurgeon can hold multiple repository states in memory.  Each has a
...
[[read_cmd,read]]
// COMMAND
include::docinclude/read.adoc[]

// COMMAND
include::docinclude/write.adoc[]
...
// COMMAND
include::docinclude/import.adoc[]
...
// COMMAND
include::docinclude/clone.adoc[]

The "rename repo" mode of <<help_cmd>> can be used to rename
a reoopository.
...
// COMMAND
include::docinclude/create.adoc[]
...
// COMMAND
include::docinclude/reparent.adoc[]
...
+
As a convenience, if ATTR-SELECTION is empty or not
...
existing matching NAME.  Likewise, for DATE.
+
As a convenience, if ATTR-SELECTION is empty or not
...
// COMMAND
include::docinclude/prepend.adoc[]
...
[[paths]]
=== Path reports and modifications
...
just groups of digits separated by dots with no identifying prefix.
...
find possible CVS and Subversion references with the `=N` visibility
set; then replace them with equivalent delimited cookies; then run
`<<stampify_cmd>>` to turn the cookies into action stamps (using the
information in the legacy-reference map) without having to do the
lookup by hand.

// COMMAND
include::docinclude/stampify.adoc[]
...
CVS, Subversion, Mercurial, and Fossil do not have separated notions
of committer and author for changesets; when lifted to a VCS that
does, like git or bzr/brz, their one author field is used for both.
...
There are two different way to package command sequences for
...
coversion recipes as you develop them.
...
The "set eadlimit" option can be used to limit processing to
head segments of an input stream file; this can be useful for
profiling andf fault isolation.

Profiling commands are:

// COMMAND
include::docinclude/show.adoc[]

// COMMAND
include::docinclude/checkpoint.adoc[]

// COMMAND
include::docinclude/profile.adoc[]
...
differences occasionally lead to conversion problems.
...
lightweight tag to point at the tip.
...
There's another problem around Subversion merges. In a DVCS, a merge
...
If a stream file has gaps in the revision sequence, branch copy
references to nonexistent revisions will be patched to refer to the
most recent existing revision before the missing one.
...
* '```trunk```' always becomes the master
branch.  Directories under '```branches```' become gitspace branches;
directories under '```tags```' become gitspace tags.
...
named '```unbranched```'. If the Subversion repository has a branch
...
Occasionally (and usually by mistake) a branchy Subversion repository
...
have IDs 2317-1, 2317-2, and 2317-3.
...
Any _.cvsignore_ files left over from a Subversion repository's
...
Subversion local usernames are mapped in the style of `git
cvs-import`; user '```foo```' becomes '```foo <foo>```', which is
sufficient to pacify git and other systems that require email
addresses. You can remap thwaw to real addresses using the
...
By convention, subversion repository is supposed to have a regular
...
A repository in standard layout can be converted nearly losslessly into
Git and other modern DVCSes, with minor exceptions around deleted and
later recreated branches and some Subversion-specific file properties.

But standard layout is only a convention; the Subversion tools don't
actually enforce it.  One very simple non-standard layout is a "flat"
repository.  We'll discuss these in detail in a <<flat,later
section>>.

In a more common non-standard layout, Subversion repositories are
...
having a standard layout underneath it.

We'll call this kind of layout "conformable" when it is perfectly
regular, not containing any of the following exceptions:

* Top-level project directories that _don't_ have a standard
layout underneath

* Copy operations on top-level subdirectories.

* trunk/tags/branches structure more than one level beneath
the top level.

These sorts of irregularities do not fit well into reposurgeon's
...
Unfortunately, it is rare for a multiproject repository not to have
...
swapcheck" for more details.

Conversions of non-conformable multiproject Subversion repositories
...
in-house to climb the learning curve.

The general strategy in these cases has three steps:

. Transform a stream dump from the repository into a fully-conformable
  one.

. Apply repocutter swapsvn to transform that stream to one
  with standard layout.

. Load the standard-layout stream into reposurgeon and trim out
  unneeded content inside reposurgeon.

The first step is the tricky one. There are two general tactics you
can pursue towards this: (1) expunging badly-structured
content, and/or (2) using repocutter pathrename to massage
non-conformable directories into conformable ones.

Before you think about doing expunges, be aware that they can be
...
Generally you're going to do the heavy lifting with repocutter
...
you need to do those you should reconsider not hiring the author.

It cannot be emphasized enough that conversions of non-conformable
multi-project repositories are *difficult*; expect your timeframe to
...
==== Some caveats about repocutter swapsvn

All project-local tags and branches will be promoted to become tags
and branches across the entire repository.

Paths that swapsvn does not recognize as conformable will be passed
through unaltered.  Don't try to treat this as a feature;
reposurgeon's fallbacks for dealing with these are crude and fragile.

Subversion mergeinfos, being file- and directory-oriented rather than
...
discard those properties to get a clean conversion.

See the embedded help listed by "repocutter help swapsvn" for some
details and caveats.

[[flat]]
=== Flat repositories

If the repository has no branch structure at all, you might want to
filter the stream with "repocutter push trunk" and then individually
turning subdirectories into branches using the <<branchlift_cmd>>
command.  The pattern for that looks like this:
...
This method has the disadvantage that you have to enumerate all
...
If for some reason you want to treat a branchy repository as though
it's flat, using repocutter to rename the "trunk" directory to
something else first (like, say, "main") will foil the logic in the
reposurgeon stream reader that wants to do branch and tag analysis.

[[CVS]]
== Working with CVS (cvs)
...
Glitch #2: In cleaning up a CVS conversion that is unique to that
...
delete junk branches and tags; reposurgeon(1) makes this easy to do.
...
There are some important caveats about hg. Please read this section
carefully before attempting conversion or surgery.

In hg the "branch" of a commit is a name carried in the commit
...
in a directory layout (as in svn).

When different people add commits to a branch, and then
...
other than the hex IDs of their tip commits.

Typically someone will then merge the changes from all the heads into
following commits on their branch.  There is an
https://wiki.mercurial-scm.org/BranchingExplained[illuminating
diagram] of this.

Before importing a Mercurial repository, *you should merge-resolve
all branches so they have single heads* first. The extractor can't
...
has an analog multi-headed branches.

In hg branches can have a "closed" status marking that line of
development as terminated. This flag is not represented in reposurgeon
abd can't be exported to other VCSes.

/////////////////////////////////////////////////////
Date: Wed, 20 Dec 2023 01:53:58 +0000
From ec429@cantab.net  Tue Dec 19 21:03:27 2023
From: Edward Cree <ec429@cantab.net>
To: "Eric S. Raymond" <esr@thyrsus.com>
Subject: Re: How does the extractor handlke branches with multiple heads?

On 20/12/2023 00:42, Eric S. Raymond wrote:
> You seem to be the person who contreibuted actual knowledge of
> Mercurial to repoosurgeon's Mercurial extractor class. So I
> have a question.
>
> What does it do with Mercurial branches that have multiple heads?

If I ever knew this for certain I've long since forgotten, sorry.
My best guess, based on re-reading the code and a few tests in a
toy Hg repo, is as follows:
Every commit will get branch coloured according to which branch
Hg says it belongs to.
refs/heads/ will contain only one head for each branch, whichever
one 'hg branches' outputs (I think this is the most recent head
on the branch in the linear local ordering, which is horrifically
arbitrary but hey that's Mercurial).
Consequently the other heads will not be reachable from any
branch ref; user will probably want to create new branches from
them otherwise they'll get GCed by git.

If you wanted to improve this I guess the starting point would be
to run
    hg heads -T "{node|short}\t{branch}\n"
to generate a list of all heads and the branches they're on, and
then create disambiguated branch names so you could give every
one of them a ref.

I don't think I considered this issue at the time because getting
multiple heads into your central Hg repo (rather than individual
developers' clones) takes some especially bad practice, and it
should always be possible to fix it in Hg with a dummy merge.
/////////////////////////////////////////////////////

There is another feature in hg that resembles named branches:
bookmarks - symbolic namesa for branch-tip commits that you assign, which
won't be identical to the name of the branch you're on unless you set
rthe that way, and which move to the new tip on the branch every time
you add one. There's a https://wiki.mercurial-scm.org/Bookmarks[detailed
description].

By default, bookmarks are ignored in favor of the name of the branch
they are on. You can specify explicit handling for bookmarks by
setting '```reposurgeon.bookmarks```' in your _.hg/hgrc_. Set the
value to the prefix that reposurgeon should use for bookmarks.
...
Mercurial tags are name-to-commit mappings stored in a (versioned)
dotfile in your repository directory. They are exported to other
VCSes as lightweight tags.
...
=== Use the Q set to check your work

When you're experimenting with mass changes based on pattern matching
...
can use this as a selection with "list" commands to check your work.

=== Pushing and popping pathname segments

Because "/" is the default delimiter for regular expressions, it may
not be obvious how to write commands that have to manipulate
pathnames by segment.  Here is how to handle some common cases:
...
The second recipe uses the fact that almost any delimiter character
...
You also can't use double quotes, as those are striped off argument
tokens before they are passed to the functionss that interpret them.
Thus , in the following command...

-----------------------------------
rename path "foo/(.*)" "${1}"
-----------------------------------

...the first argument would be interpreted as a literal path name and
not a regexp. However, this eould work as probably intended:

-----------------------------------
rename path ":foo/(.*):" "${1}"
-----------------------------------

That is, the double quotes will be stripped off at argument parsing
time and then the matching colons will cause the first argumment to be
recognized as a delimiyed regexp. This will be useful if you need to
specify a delimited regexp that contains whitespace; you can
string-quote it so the argument parser will pass it as a single token.

=== Extracting the history of a single file

You can use a negative selection to extract the history of a single
file. Using the reposurgeon repository itself as an example:
...
After thus rommands, the reposurgeon-clone repository will
contain only the history of ci/prepare.sh, with the "ci" directory
popped off the filenamw in the way indicated in the previous tip.

=== Use macros to avoid repeating boilerplate

We can encapsulate that technique for stripping off a directory as
follows:
...
"```++*** empty log message ***++```" that modifies only a ChangeLog
entry explaining the commit immediately previous to it. The following
...
=== Handling username collisions

Occasionally when processing a CVS or Subversion repository you will
...
full name-and-email attributions you want to apply.

As an example, suppose your history has two committers named
...
with variable data following.

There's no way you can discriminate by time in an authors file.
But there's a different technique that will work.  Like this:
...
The third command allows you to check that the previous two produced
a sane-looking result.

Part of the trick here is knowing that if you only set name and email
fields with an attribute command the date field is left unchanged.

If you needed to handle more than one of these collisions, you
might consider a slightly different approach that illustrates
how to write and use a multiline macro:
...
This depends on the fact that the the script arguments %{0} and %{1} get
...
=== Use reposurgeon.el to cut down on repetitive handwork

An Emacs mode for editing msgout dumps ships with the reposurgeon
distribution.  It includes various useful commands for Gitifying
comments and reference-lifting.

=== Avoid git-svn lossage with repocutter filtering

Subversion histories that have been touched by git-svn often have
...
repository. Commits on these paths are best treated as junk and removed.

While they could be removed with a reposurgeon delete path command after
...
actual Subversion part of the history being undamaged.

If you suspect this is happening, the following repocutter expunge
...
If you use this to pre-filter your repository dump it will remove the
...
=== Prevent timestamp collisions with timequake

If you're going to use msgout/msgin to batch-edit commit metadata,
...
After running timequake it's a good idea to do "=Q list" to check
that it hasn't had surprising effects.

[[troubleshooting]]
== Troubleshooting and bug reports

[[reposurgeon-crashes]]
=== When reposurgeon crashes

If reposurgeon crashes on a Subversion stream dump that has not been
altered or filtered in any way since being emitted by svnadmin dump,
that is definitely a bug; please <<reporting-bugs,report it as such>>.
To speed up the fix, follow the instructions on preparing a reduced
test case.

You are more likely to see a crash when processing a stream that has
beem modified by filtering out unwanted commits, e.g. via select or
...
the the reposurgeon maintainers have not successfully characterized.
...
1. Identify and report the phase in which the stallout occurs.  Be
   ...
   more RAM and compute clocks at the problem.
...
Be aware that bugs can be due to problems in any of (a) reposurgeon
...
read
https://gitlab.com/esr/cvs-fast-export/-/blob/master/reporting-bugs.adoc[Reporting
bugs in cvs-fast-export] and follow those directions.
...
added afterwards can confuse diagnostics.
...
"<<repotool.adoc#,repotool>> initmake", mention that in your bug
...
reduce the bulk of the test case you send.
...
There is a cvsstrip tool in the cvs-fast-export distribution
...
distribution to lean how to use it.
...
'expunge' operation on repocutter or the '<<delete_cmd>> branch'
...
In versions 4.38 and earlier, the DSL was substantially different The
...
The =E event selector didn't yet exist.  There was a read --use-uuid
...
bnecomes "attribute". The syntax of the "remove" command was dufferent
and more compolex. The "import" command was "incorporate".

In versions 4.30 and earlier, the read command had a --branchify
option. This has been retired, you are now expected to do the
equivalent manipulations with repocutter.

The "tip" command present in 4.24 and earlier has been removed,
...
In versions 4.24 and earlier the behavior of the "unite" command
was incompatibly different in that it took master branch from the last
repo in the list, renaming others. Now it leaves master from the first.

The blob command now takes an explicit mark, rather than creating a new
blob :1 and renumbering all others as in 4.23 and earlier.

Filter syntax now uses subcommand verbs rather than the options of 4.23 and
earlier.

In versions 4.23 and earlier, the stats command operated on named
repositories rather than the currently-selected one.

In versions 4.23 and earlier, it was possible to redirect output from
...
In versions 4.23 and earlier, "<<branchify_opt>>" read
...
its cases in a simpler way.

In versions 4.23 and earlier, the behavior of the branch command was
incompatibly different; it did not require a "heads/" or "tags/"
prefix on its operand and, because of that, could only operate on
heads/ branches and not lightweight tags.

In versions 4.23 and earlier, several commands that now have the form
"object verb selection" had the form "object selection verb".  This
includes branch, tag, reset, and path.

In versions 4.23 and earlier, the syntax of the expunge command
was different; it used "~" instead of "--not" and took multiple
patterns.

The "paths sup" and "paths sub" commands of versions 4.23 and earlier
have been retired and replaced by the enhanced path rename command.

As of 4.23, branch references as used in "branch rename" and elswhere
...
In versions before 4.8, the `expunge` command run on a
...
named "mailbox_in" and "mailbox_out:"; `--branchify` was
...
If you need emergency help, go to the #reposurgeon IRC on irc.oftc.net.
...
Also accepted is the bzr/brz syntax for setting per-commit
...
Unlike other extensions, bzr/brz properties are only preserved on
stream output if the preferred type is bzr or brz, because any importer other
than those will choke on them.
...
Guarantee: reposurgeon handles the bzr/brz commit-properties extension,
...
Limitation: bzr/brz suffers from deep confusion over whether its unit of
...
The syntax of the reposurgeon DSL evolved ad hoc as more capabilities
...
Wrestling the DSL into a simpler and more uniform shape is an ongoing
area of development, and not a simple one. There are conflicting
objectives:

. Simplify individual commands with complex syntax.

. Most commands have the form of a simple verb. (V). Some have the
  form verb-subject (VS) or verb-subject-object (VSO). Some have the
  form subject-verb (SV). Some are subject nouns (S).
  We want the simplest and most uniform morphology possible.

. Group commands so the language is easier to remember and document.

. Use subcommand keywords for alternative modes, as opposed to
  policy bits that can be set several at a time.

. Keep commonly-used commands short and simple.

A relatively recent advance at the end of the 4.x release
...
replaced differeng tokenization rules in different commands.
...
The following are SV. Inverting these to verb-first might be a good
thing. Unfortunately this would make the embedded help for the read
and write verbs a big heterogenous mess.
...
The following are S, and there's no obvious way to
move them to a verb-first form:
...
Eric Sunshine <sunshine@sunshineco.com>::
   ...
   Added '<<attribute_cmd>>' command.
   Added '<<reorder_cmd>>' command.
   Assisted Python to Go port.
...
https://github.com/lelit/tailor

Tailor aimed to be an any-to-any repository translator.
As of late 2023 the last commit was in 2011.
...
!LINEAR, !NO_TAGS, !MIXEDBRANCH, CONFIGURATION, ABANDONED.

If you could not use reposurgeon for some reason, this was one of
the best alternatives. However, as of 2023 it appears to have
been abandoned in 2016.
...
If you could use reposurgeon for some reason, this was another
alternative that was not too horrible (but beware of possible
history damage if your Subversion repo has malformations that
confuse git-svn).  As of 2023 it seems to have been abandoned
since 2016.
...
=== svn-all-fast-export (nyblom version)

https://github.com/svn-all-fast-export/svn2git

Written in C++. Says it's also named svn2git and might be genertically
...
very elaborate configuraion to make Subversion branch structure to
git branch structure. Actively mauntained in 2023.

CONFIGURATION, !ABANDONED.

=== svn-fast-export

Formerly at http://c133.org/code/svn-fast-export.c
...
===  svn-all-fast-export (thiago version)

http://repo.or.cz/w/svn-all-fast-export.git

May be genetically related to the Nyblom svn2git, but if so they
diverged in 2008. As of 2023 

LINEAR, NO_TAGS, NO_IGNORES, NO_DOCUMENTATION, ABANDONED.

=== sccs2git

There is a script called sccs2git on CPAN which attempts to lift SCCS
