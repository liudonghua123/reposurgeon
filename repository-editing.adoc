...
"branchlift"). Instead, you may be able to use the repocutter filter
to transform the dump file into a version shaped right for a regular
branch-sensitive lift.
...
surgery, with e.g. the `branchlift` command. Note that this option
will disable partitioning of mixed commits.
