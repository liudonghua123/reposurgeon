...
An optional argument prefixed by "<" indicates that the command
accepts input redirection; an optional argument prefixed by ">"
indicates that the command accepts output redirection.
...
directs the output of a list command to more(1).
...
level. This transformations is performed when the actual read of the
repository happens.  

The following branch renames transform these branches into a
standard layout.  Tag creations do not need to be mapped separatrely,
as the genmerated gitspace tags are moved when their base branch is.
...
option was a separate commands that needed to be invoked before the
read and set hidden global context.  There was a related branchmap
commannd thast has been retired becuse branch rename covers all
