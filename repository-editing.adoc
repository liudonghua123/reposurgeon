...
branches or tags made with CVS version vefore before CVS got commit
...
the rate of over 400K commits per minute, but that rate can fall off
...
Tags from CVS -root commits::
    Branch root markers aee made by CVS for internal purposes. These
...
[ SELECTION ] authors [ read | write ] [ <INFILE ] [ >OUTFILE ]::
