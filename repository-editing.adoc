...
repository madter directory, and run reposurgeon pointed at that
director or one of its submodules; a checkout directory won't do.
...
The command `=L <<msgout_cmd>>` is good for extacting commits with
multi-line comments in a form that cann be deited and merged back in
...
Following the rebames, the unneeded project2 branch can be dropped.
...
Filter syntax now subcommand vers rather than the options of 4.23 and
earlier.
...
removed; msgin now sets =Q bits which can be used to generatew the
same report.
...
was different; it use  "~" instad of "--not" and took multiple
patterns.
