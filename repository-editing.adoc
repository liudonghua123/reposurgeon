...
bzr, brz, darcs, RCS, and SRC. There is tested read-side support for
SCCS, CVS and svn. There is untested read-write support for Bitkeeper
and Fossil, and untested read support for Monotone.
...
unversioned ignore-globs set through the Web interface. It also leaves
...
Bazaar and Breezy (bzr, brz): These are workalikes. Read support is
tested, write support is not.
