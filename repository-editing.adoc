...
switch convebtions.  Each string in the command line argument is
passed to the interpreter as a command, in order; thus, you could
ewplicate the first two action of the previous script with this:
...
As stacking up read options can resilt in a very long read invocation
line, it;'s useful to know that \ is sccepted as a comtinuation
chracter.  Thus, you can do something like this, which is an actual
...
the `<<branchify>>` read option for how to alter this set, which
...
`<<branchify>>` as a namespace.  If `*` is in the branchify set
...
and `<<branchmap_cmd>>`. If the Subversion repository has a branch
...
In versions 4.22 and rearlier, the "--branchify"
and "--branchmap" read options were separate commands that needed
to be inoked before the read and set hidden global context.
