...
Help is available within reposurgeon using the "help" command.
...
`--format=fossil` option of the '```write```' comment. Ignore patterns
...
As a motivating example, here are commands to do a Subversion-to-Git
...
this usually means converting from something else to Git.
...
Run "repotool initialize" in the scratch directory (it requires that
you follow the initialize verb with a project name).  This will create
...
one module, see <<CVS, this important caveat>>.
...
[[zombie]]
There are two kinds of non-serious CVS conversion glitches: file
content mismatches due to keyword fields in masters, and 'zombie'
files deleted in CVS that get resurrected in Git revisions
...
To the author's knowledge, reposurgeon is the only conversion tool
that handles multibranch Subversion repositories in full
generality. It can even correctly translate Subversion commits that
alter multiple branches.
...
branches that may need manual correction.
...
+
Additional options to this command and not listed here are given in a
later section, and apply only to Subversion repositories.
+
...
This means that if you do a write to a directory, it will build a repo of
...
contents of the new repository are checked out (but see the
...
method and you have to set preservations by hand.
...
`unpreserve` [ _file..._ ]::
   Remove (presumably untracked) files or directories from
...
[ _selection_ ] `reparent` [ _options_... ] [ _policy_ ]::
...
{ _selection_ } `split` {`at`|`by`} _item_ ::
    ...
    with the pathname match done first.
+
...
the '`__perm__`' part must be a token ending with `755` or `644`, and the
'`__mark__`' must refer to a blob that precedes the commit location.  For
an *R* or *C* operation to be valid, there must be an *M* operation for
the source in the commit's ancestry.

{ _selection_ } `remove` [ _index_ | _path_ | `deletes` ] [ `to` _commit_ ]::
...
an existing tag).
+
For a '```delete```', no third argument is required.  The name portion of
a delete may be a regexp wrapped in `//`; if so, all objects of the
...
[ _selection_ ] `expunge` [ `--notagify` ] [~] [ _path_ | /__regexp__/ ]...::
   ...
   string quotes snd backslashes is performeed when parsing this command 
...
fileops pointing both in and outside the path set are not deleted, but
are cloned into the removal set.
...
separated by '```+```'. It will have no source directory or preferred
system type.
+
...
that only files with properly matching *M*, *R*, and *C* operations in
the root survive.
...
commit in the currently chosen repo.
+
Labels and branches in the named repo are prefixed with its name; then
it is grafted to the selected one. Any other callouts in the named repo
are also resolved in the context of the currently chosen one. Finally,
the named repo is removed from the load list.
+
...
modification is aborted. This helps ensure that you are landing updates
of the events you intend.
+
...
+
The special field names '```author```', '```commitdate```' and '```authdate```'
apply only to commits in the range.  The latter two set attribution
...
[ _selection_ ] `append` [ `--rstrip` ] [ `--legacy` ] [__text__]::
   Append text to the comments of commits and tags in the specified
   selection set. The text is the first non-option token of the
   command and may be a quoted string. C-style escape sequences in the
   string are interpreted as one would expect.
+
...
`to` being passed as arguments to the standard re.sub() function to
modify the content. Actually, any non-space character will work as a
delimiter in place of the `/`; this makes it easier to use `/` in
patterns. Ordinarily only the first such substitution is performed;
putting '```g```' after the slash replaces globally, and a numeric
literal gives the maximum number of substitutions to perform. Other flags
available restrict substitution scope - '```c```' for comment text only,
'```C```' for committer name only, '```a```' for author names only.  Note
that parsing of a `--regex` argument will be confused by any substring
consisting of whitespace followed by `#`; use '```\s```' rather than
whitespace to avoid this.
+
...
[ _selection_ ] `strip` [ `--blobs` | `--reduce` ]::
...
reducing large repositories to test cases of manageable size.  You can
do a repocutter "obscure" pass after the strip to mask even the metadata.
+
...
branch with any given name, so the view back from any branch tip in the
repository has the correct content. This does however mean that
reposurgeon discards the content of any branch having that same name.
...
`--user-ignores`::
By default reposurgeon filters in-tree _.gitignore_ files found in the
history because they would clash with those generated from the
`svn:ignore` and `svn:global-ignores` properties.  Using this option
makes _.gitignore_ files be passed through. They will still be
overridden by generated _.gitignore_ files so this option is often
used along with `--no-automatic-ignores`.
...
branches.  The reference is disambiguated using the base revision
number of the dead branch. (Note that the reason --preserve is not the
default behavior is because of experience with large old repositories
that may have hundreds or even thousands of dead branches. While it is
important that content copies fron dead branches be resolved
correctly, the branches themselves are almost never interesting.)

`--cvsignores`::
Suppress the normal deletion of _.cvsignore_ files.

These modifiers can go anywhere in any order on the command
...
which is mapped to `master`, and `/tags` and `/branches` which are
...
_.gitignore_ files, and the `--no-automatic-ignores` which discards
...
user-created .gitignore ignored::
   This message means reposurgeon has
...
These commands control the branchify set, defined earlier, and how
branch and tag names are to be rewritten as they are read from the
...
+
List elements are old-fashioned path glob patterns; each * matches any
...
`branchmap` [ _/regex/branch/_ ... | `reset` ] ::
   ...
   branches that are detected by `branchify` to their rewritten names.
   If none of the expressions match a particular branch, the default
   behavior applies. This maps a branch to the name of the last
   directory, except for `trunk` and `*` which are mapped to `master` and
   `root`.
+
...
the command line, as this would clash with the use of backslashes as
...
branchmap :project1/trunk:heads/master: :project1/tags:refs/tags: :project1/branches:refs/branches:
set testmode
...
to changesets, and conversions have the option of marking each target
...
that the "history" command exists and can dump your command history
to a file.

(d) If you were using the standard-workflow Makefile generated by
"repotool initialize", mention that in your bug report.  If you
modified the Makefile, include a copy with the bug report.
...
repocutter "obscure" pass after the strip. This will mask file paths
and developer names.
...
reproduces.  If it does, repeat; if it does not, throw out the last
...
command were bare keywords. There was a command to set the prompt
string that has been retired.
...
svn-all-fast-export, but if so they diverged in 2008. Noteable for
...
GitHub]. This version has been extened and modified somewhat, and some
...
Reposurgeon is intended to be hackable to support for special-purpose or
...
representations of the command types in an import stream, with
...
support for those strams was estimated to be lower than that
