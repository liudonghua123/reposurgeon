...
is a place where the Subversion model of tags clashes destructively with the
...
the `<<branchify_cmd>>` command for how to alter this set, which
...
`<<branchify_cmd>>` as a namespace.  If `*` is in the branchify set
...
another directory to master using `<<branchify_cmd>>` and
`<<branchmap_cmd>>`. If the Subversion repository has a branch named
'```unbranched```' the name '```unbranched-bis```' is used instead;
actually, '```-bis```' is appended enough times to get to an unused
branch name.
...
This method has the disadvantages that (1) you have to enumerate all
branches, (2) there is no authomatic tag processing, and (3) there
is no authomatic processing of what would correspond to split-branch
commits; branchlift will fail with a message rather than try to
produce one.  Still, it may be useful on repositories that consist
of one big file tree not conforming to a standard layout.
...
named "mailbox_in" and "mailbox_out:"; `<<branchify_cmd>>` was
