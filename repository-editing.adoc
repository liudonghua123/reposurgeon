...
write repositories) include git, hg, bzr, brz, darcs, bk, RCS, and
SRC.  For a complete list, with dependencies and technical notes, type
...
--------------------------
$ reposurgeon "script projectscript.lift"
--------------------------
...
http://www.catb.org/~esr/cvs-fast-export[cvs-fast-export] for CVS, or the
VCS tool itself for Subversion and other systems.
...
Reposurgeon can read CVS repositories.  You must have a copy of the
repository master directory, and run reposurgeon pointed at that
directory or one of its submodules; a checkout directory won't do.
...
such as some projects use for website content.
...
stream to reposurgeon;s importer.
...
Perforce: repotool can be used to mirror a remote p4 repository as a
...
The reposurgeon command `=H <<inspect_cmd>>` will show you tip commits
...
use "=Q list" and "=Q inspect" to zero in on anomalies reported by
lint.
...
[[read_cmd,read]]
read [--quiet] [ --format=fossil ] [ DIRECTORY | - | <INFILE ]::
...
_.gitignore_/_.hgignore_/_.bzrignore_/.brzignore file in other systems) and moves
...
For bzr (Breezy) see the bzr entries.
...
equivalent delimoited cookies; then run `<<references_cmd>>` to turn the
...
These commands control the level and destination
...
profile [live|start|save|bench] [PORT | SUBJECT [FILENAME]]::
    ...
    given it prints out the available types of profiles.  '```profile
    bench```' has an internal helper for the repobench script. There
    is more detailed documentation on this command in the embedded help.
...
can use this as a selection with "list" or "inspect" commands to check
your work.
...
----
1..<2010-90-15T00:00:00Z> & /^fred$/C attribution =C|=A set "Fred Foonly" "fred1@foonly.org"
<2010-90-15T00:00:00Z>..$ & /^fred$/C attribution =C|=A set "Fred J. Muggs" "fred2@foonly.org"
/fred[12]/C inspect
----
...
fields with an attribution command the date field is left unchanged.
...
1..<{1}> & /^%{0}$/C attribution =C|=A set "%{2}" "%{2}"
<{1}>..$ & /^%{0}$/C attribution =C|=A set "%{3}" "%{3}"
}
...
In versions 4.38 and earlier, there was no "view" or "clone" command.
The command-line parser was fragile and ad-hoc, without consistent
application of double-quoting. The =E event selector didn't yet exist.
There was a read --use-uuid option which has been retired. Negation
was not yet supported in pathsets. The regex, path rename, branch
rename, and tag rename commands used Python-style rather than Go-style
match references. Macro argument formals did not yet have a %
prefix. The split command syntax was incompatibly different.  The
"set" and "clear" commands didn't take a subcommand verb.  There was a
"readlimit" command doing what "set readlimit" now does. The
"elapsed", "memory", and "sizeof" subcommands of "show" used to be
separate commands. Blob/tag/reset creation used to be
"{blob|tag|reset} create", not "create {blob|tag|reset}";
blob/tag/branch/reset deletion used to be "{blob/tag/branch/reset}
delete", not "delete {blob/tag/branch/reset}".  The command to rename
repositories was just "rename", not "rename repo"; "rename path" was
"path rename"; "renane reset" was "reset rename". The "delete path"
command was named "expunge".
...
The syntax of the reposurgeomn DSL evolved ad hoc as more capabilities
were glued onto the tool. The result is something of an irregular mess
...
. Most commands have the form of a simple verb. (V). Some have the
  ...
  form subject-verb (SV). Some are subject nouns (S). Some are harder
  to classify. Ideally we want a simpler and more uniform morphology.

. Group commands so the language is easier to remember and document.

. The distinction between things done with policy flags vs. things
  done with subcommand ketwords is not very principled. Use subcommand
  keywords more for alternative modes, as opposed to policy bits that
  can be set several at a time.

. Keep commonly-used commands short and simple.

. Avoid using the same verb for VS commands that may have
  a selection set and must not have a selection set.
...
Most commands are simple and V:
...
[SELECTION] dedup
define [NAME [TEXT]]
{SELECTION} delete [--quiet] {commit | {path|tag|branch|reset} [--not|--notagify] PATTERN}
...
quit
rebuild [DIRECTORY]
rename {repo | path PATTERN [--force] | {branch|reset|tag} [--not] PATTERN}} NEW-NAME
renumber
...
The following are VS and unpleasantly complex - there are
alternations in the syntax that aren't just flags.
...
The following are SV. Inverting these to verb-first might be a good thing.

----
[SELECTION] authors {read <INFILE | write >OUTFILE}
legacy {read [<INFILE] | write [>OUTFILE]}
path list [>OUTFILE]
[SELECTION] reset {move|rename} [--not] RESET-PATTERN [NEW-NAME]
[SELECTION] tag {move|rename} [TAG-PATTERN] [--not] [NEW-NAME]
[SELECTION] attributions [ATTR-SELECTION] SUBCOMMAND [ARG...]
----

The following are S and should probably be attached to some verb:
...
The following are S, but probably shouldn't change:
...
Should these be subcommands of list?
...
In a revised, more verb-centered syntax, the key verbs would probanly
be: list read write create move rename delete add.  Here are some
opossible verbifications:
...
read message <INFILE
read repo <INFILE
write authors >OUTFILE
write legacy >OUTFILE
write message >OUTFILE
write repo >OUTFILE
---

Unfortunately these would make repository loads unpleasantly verbose,
cpnflicting with "short and simple for common commands".
