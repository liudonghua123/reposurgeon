...
losslessly captures Git histories.

Git has top-tier support with excellent testing.

=== CVS

Reposurgeon can read, but not write, CVS repositories.  You must have
a copy of the repository master directory, and run reposurgeon pointed
at that directory or one of its submodules; a checkout directory won't
do.
...
structures; reposurgeonn cannot entirely compensate for these flaws.
See <<CVS>> for discussion and workarounds.

CVS has top-tier support with good testing.
...
Subversion has top-tier support with excellent testing.

=== Mercurial

Mercurial (hg): reposurgeon can read a Mercurial repository using
either of two methods, and write one using a Mercurial plugin.  See
<<mercurial>> for details. Reposurgeon has been used for
successful history conversions from Mercurial to Git.
...
Bazaar and Breezy (bzr, brz): These are workalikes. Import has only basic
