...
CVS has top-tier support with good testing.  It is, however, not
expected that write to CVS will ever be implemented. (This means:
somebody would have to pay the maintainer real money to make it
happen.)
...
Subversion has top-tier support with excellent testing. It is,
however, not likely that write to Subversion will ever be
...
repositories.  See <<mercurial>> for details. Reposurgeon has been
used for successful history conversions from Mercurial to Git.
...
Bazaar and Breezy (bzr, brz): These are workalikes. Import and export have only basic
test coverage, but reposurgeon has been successfully used for a major
Bazaar-to-Git conversion (the history of Emacs).
