...
Before importing a Mercurial repository, you should resolve
all branches so they have single heads. The extractor can't
handle multiple-headed branches gracefully.

Mercurial branches are exported as branches in the exported
repository, and tags are exported as tags.
...
Subject: Re: How does the extractor handlke branches with multople heads?
...
By default, bookmarks are ignored. You can specify explicit handling
for bookmarks by setting '```reposurgeon.bookmarks```' in your
_.hg/hgrc_. Set the value to the prefix that reposurgeon should use
for bookmarks.
