...
==== Whole-repository conversion with repocutter swapsvn

The preferred way to handle multiproject repositories is with the
repocutter "swapsvn" primitive.  If the multiproject structure is
perfectly regular - top-level directories are always projects, and
always have trunk/tags/branches structure immediately underneath -
swapsvn will transform the repository to a regular Subversion
structure with trunk/tags/branches at the top level.

Be aware that if you do this, all project-local tags and branches
will be promoted to become tags and branches across the entire
repository.

Sometimes you may be able to transform a nearly-regular multiproject
structure into a perfectly regular one with repocutter pathrename
commands, then apply repocutter swapsvn.

Paths that swapsvn does not recognize as transformable will be passed
through unaltered.
