...
If you get a very long list of test report lines, install tappy or
install some other consumer of TAP (Test Anything Protocol) and set it
as TAPCONSUMER in test/Makefile.  This will make the output far less
noisy, passing though only errror indicattions. You can get the raw
TAP report at any timw with "make tap".
