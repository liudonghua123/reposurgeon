...
Output from the tests is in TAP (Test Anything Protcol).  Test output
...
as an empty dirctory and filled in with file copies in later revisions.
...
FIXME: resort needs a test, so does the cut-failure case of divide.
