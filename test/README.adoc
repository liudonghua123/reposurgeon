...
If you get a very long list of test report lines, install tappy; get
it with "apt get tappy" in Debian-based Linuxes, or see the first URL
in the references below. Alternatively, you could install some other
consumer of TAP (Test Anything Protocol) and set it as TAPCONSUMER in
test/Makefile.

This will make the test output far less noisy, passing though only
error indications. You can get the raw TAP report at any time with
"make tap".

On the 64-core 3GHz AMD64 machine I use for development, a full
make check runs in 15-20 seconds. Scale expectations appropriately
for your hardware.
...
All reports from test scripts are in the form of TAP (Test Amything Protocol).
